...
\begin{frame}{Introdução}
    \begin{itemize}
        \item Analisamos algumas abordagens dos erros mais frequentes encontrados em nossa aplicação
        ...
        \item Vamos exemplificar um dos tipos de erros mais comuns que fora encontrado
    \end{itemize}
\end{frame}

\begin{frame}{Erros de fluxo}
    \begin{itemize}
        \item Erros de fluxo podem ser causados por erros na implementação das regras de negócio ou no tratamento indevido de falhas que podem desviar o fluxo conforme o esperado para a aplicação
...
\begin{frame}{Resultados orientado a tipos}
    \begin{itemize}
        \item Imagine que um novo tipo \emph{NetworkFail} implementa \emph{Result}, mas contendo outros dados sobre a falha
        \item Em Java, não existe um meio de garantir que todos os casos sejam cobertos
        \item Seu compilador não obriga o desenvolvedor a tratar este novo tipo de resultado
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Resultados orientado a tipos + Pattern Matching}
    \begin{itemize}
        ...
        \item Utilizando a expressão \emph{when}, é possível implementar uma espécie de \textit{pattern matching}, que nos obriga é a cobrir todas as implementações possíveis de um determinado valor
        \item Complementando o exemplo em Java, tome para um exemplo uma classe \emph{sealed} chanda \emph{Result} com as implementações que representam um resultado de sucesso e uma de erro
        \item Quando uma nova implementação, o compilador falha em todas as verificações da expressão \emph{when}, pois, estas não serão exaustivas (isto é, não cobrem todas possibilidades)
    \end{itemize}
\end{frame}

\begin{frame}{Conclusões}
    \begin{itemize}
        \item Podemos detectar em tempo de compilação alguns erros mais triviais e de grande risco, como os erros de variável não inicializada
        \item A possibilidade de não captura ou tratamento de eventos inesperados diminuem empregando alguns destes tipos de abordagens
        \item Além disso, o risco de durante o ciclo de vida aplicação de se propagar novos erros é reduzido
        \item Com isso, o custo de manutenção e incremento do software é reduzido
    \end{itemize}
\end{frame}
...
\begin{frame}{Contextualização}
    \begin{itemize}
        \item \emph{Linters}, como são comumente conhecidas estas ferramentas, fazem uma análise estática do código fonte da aplicação em busca de possíveis erros, adoção de má práticas de programação, problemas de formatação, entre outros
        ...
        \item Possui um conjunto de regras de análise de código para avaliar diferentes segmentos de vulnerabilidades, sendo altamente configuráveis e personalizáveis
        \item Vão além da detecção de erros, mantendo também a qualidade, estética e legibilidade do código em si
    \end{itemize}
\end{frame}

\begin{frame}{Implementação}
    \begin{itemize}
        \item Para realizar a implementação de um \textit{linter}, utilizamos um projeto inicialmente desenvolvido em Java, baseado no funcionamento da ferramenta Apache ZooKeeper
        \item Convertemos este projeto para Kotlin para possibilitar as melhorias 
        \item Além disso, com a conversão, muitos dos padrões inseguros de Java se mantem e são detectados pela ferramenta
        \item O plugin do \textit{linter detekt} foi implementado e a verificação foi executada no projeto como prova de conceito para este trabalho
    \end{itemize}
\end{frame}

\begin{frame}{Relatório exportado}
    \begin{itemize}
        \item Veja um trecho do relatório exportado em HTML após a primeira execução da ferramente em um dos módulos, como exemplo:
        \centering
...
\begin{frame}[fragile]{Injeção do \textit{dispatcher} de corrotinas}
    \begin{itemize}
        \item Abordaremos um exemplo de alerta apontado durante a análise de um dos módulos do projeto em código não conforme a regra:
        \begin{figure}[H]
            \centering
...
\begin{frame}[fragile]{Injeção do \textit{dispatcher} de corrotinas}
    \begin{itemize}
        \item Código conforme a prática recomendada
        \begin{figure}[H]
    \centering
    \begin{lstlisting}[language=Kotlin]
class ControllerImpl(
...
\begin{frame}{Conclusão}
    \begin{itemize}
        \item A execução do processo de \textit{lint} dectectou vulnerabilidades não apenas no funcionamento de aplicação, mas na observação de práticas de risco ou desencorajadas, má formatação ou estilização de código, entre dificultade de criação e manutenção de teste e outros pontos
        \item Note que este processo também é facilmente empregado no ciclo de vida do desenvolvimento da aplicação, integrando-se no processo de testes, \textit{build} e deploy de aplicações que utilizam o Gradle
    \end{itemize}
\end{frame}
...
\begin{frame}{Considerações finais}
    \begin{itemize}
        ...
        \item Aplicando esta classificação numa base conhecida, confirmamos que os erros mais comuns tem relação com o processo de desenvolvimento
        \item Analisando estes erros, podemos diminuir ou eliminar as ocorrências destes erros a curto e médio prazo
        \item Utilizando outras ferramentas como os \textit{linters}, podemos detectar e tratar vulnerabilidades de diversos tipos, através das regras nele empregadas
        \item O ganho da implementação deste tipo de ferramenta se estende ao longo de todo o tempo de vida da aplicação, uma vez que o processo de \textit{lint} pode ser executado a cada passo do processo de integração e publicação contínua
    \end{itemize}
\end{frame}
